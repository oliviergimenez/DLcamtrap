...
def comp_exif(folder):
     ...
     FN=[0]*len(id_to_class)
     dict_error_FP={}
     dict_nb={}

     os.makedirs(folder+"TP/")
     os.makedirs(folder+"FP/")
     os.makedirs(folder+"FN/")
          ...
          if file.endswith(".jpg") or file.endswith(".JPG") or file.endswith(".png"):
               ...
               if pred_class_name==None:
                    if species in id_to_class:
                         ...
                         FN[species_id]+=1
                         save_img(file_path,folder+"FN/","_"+species+"_not_detected")

               #S'il y a des predictions on prend celle avec la confidence al plus élevée
               else:
                    if pred_class_name==species: #bien prédit
                         TP[pred_id]+=1
                         save_img(file_path,folder+"TP/","_"+species)
                    else: #mal prédit : faux positif, si c'était une vraie classe il y a aussi un faux negatif
                         FP[pred_id]+=1
                         save_img(file_path,folder+"FP/","_"+pred_class_name+"_instead_of_"+species)
                         if species in id_to_class:
                              species_id=id_to_class.index(species)
                              FN[species_id]+=1
                              save_img(file_path,folder+"FN/","_"+pred_class_name+"_instead_of_"+species)
                         try:
                              dict_error_FP[species]+=1
                         except:
                              dict_error_FP[species]=1

     d = {'species': id_to_class, 'TP': TP, 'FP':FP,'FN':FN}
